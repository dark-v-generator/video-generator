...
from src.flows.publish_slots import compute_publish_slots, next_publish_slot
...
class TestComputePublishSlots:
    def test_noon_picks_today_late_then_tomorrow_early(self):
        ...
        assert result == [
            ...
            datetime.datetime(2026, 5, 5, 19, 0),
            datetime.datetime(2026, 5, 5, 20, 0),
            datetime.datetime(2026, 5, 6, 12, 0),
        ]
    ...
    def test_early_morning_picks_all_today(self):
        ...
        assert result == [
            datetime.datetime(2026, 5, 5, 12, 0),
            datetime.datetime(2026, 5, 5, 18, 0),
            datetime.datetime(2026, 5, 5, 19, 0),
            datetime.datetime(2026, 5, 5, 20, 0),
        ]
    ...
    def test_count_exceeds_daily_slots_spans_days(self):
        ...
        assert len(result) == 6
        assert result[4] == datetime.datetime(2026, 5, 6, 12, 0)
        assert result[5] == datetime.datetime(2026, 5, 6, 18, 0)
...
class TestNextPublishSlot:
    ...
    def test_after_20_wraps_to_next_day_12(self):
        after = datetime.datetime(2026, 5, 5, 20, 0)
        now = datetime.datetime(2026, 5, 5, 9, 0)
        result = next_publish_slot(after, SLOTS, min_lead_minutes=30, _now=now)
        assert result == datetime.datetime(2026, 5, 6, 12, 0)
    ...
    def test_sequential_calls_walk_forward(self):
        ...
        assert s1 == datetime.datetime(2026, 5, 5, 12, 0)
        assert s2 == datetime.datetime(2026, 5, 5, 18, 0)
        assert s3 == datetime.datetime(2026, 5, 5, 19, 0)
    ...
    def test_noon_start_picks_remaining_today_then_tomorrow(self):
        ...
        assert s1 == datetime.datetime(2026, 5, 5, 18, 0)
        assert s2 == datetime.datetime(2026, 5, 5, 19, 0)
        assert s3 == datetime.datetime(2026, 5, 5, 20, 0)
        assert s4 == datetime.datetime(2026, 5, 6, 12, 0)

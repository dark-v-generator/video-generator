...
class Caption(object):
    def __init__(
        self, language: Optional[str], sequence: int, begin: time, end: time, text: str
    ):
...
def get_captions(
    language: Optional[str], max_width: int, max_height: int, results: List[dict]
) -> List[Caption]:
...
class CaptionHelper(object):
    ...
    def get_text_or_translation(
        self, result: speechsdk.RecognitionResult
    ) -> Optional[str]:
        return result.text
    ...
    def add_captions_for_final_result(
        self, result: speechsdk.RecognitionResult, text: str
    ) -> None:
        caption_starts_at = 0
        ...
        while index < len(text):
            ...
            if add_caption:
                ...
                if is_first_caption and is_last_caption:
                    ...
                    caption_begin_and_end = self.get_partial_result_caption_timing(
                        result,
                        text,
                        caption_text,
                        caption_starts_at,
                ...
                self._captions.append(
                    Caption(
                        self._language,
                        caption_sequence,
                        caption_begin_and_end[0],
                        caption_begin_and_end[1],
                        caption_text,
                    )
                )
    ...
    def get_best_width(self, text: str, start_index: int) -> int:
        ...
        best_width = (
            remaining
            if remaining < self._max_width
            else self.find_best_width(self._first_pass_terminators, text, start_index)
        )
        if best_width < 0:
            best_width = self.find_best_width(
                self._second_pass_terminators, text, start_index
            )
        if best_width < 0:
    ...
    def get_full_caption_result_timing(
        self, result: speechsdk.RecognitionResult
    ) -> Tuple[time, time]:
    ...
    def get_partial_result_caption_timing(
        self,
        result: speechsdk.RecognitionResult,
        text: str,
        caption_text: str,
        caption_starts_at: int,
        caption_length: int,
    ) -> Tuple[time, time]:
        ...
        partial_begin = helper.add_time_and_timedelta(
            result_begin, result_duration * caption_starts_at / text_length
        )
        partial_end = helper.add_time_and_timedelta(
            result_begin,
            result_duration * (caption_starts_at + caption_length) / text_length,
        )
        return (partial_begin, partial_end)

    def is_final_result(self, result: speechsdk.RecognitionResult) -> bool:
        return (
            speechsdk.ResultReason.RecognizedSpeech == result.reason
            or speechsdk.ResultReason.RecognizedIntent == result.reason
            or speechsdk.ResultReason.TranslatedSpeech == result.reason
        )

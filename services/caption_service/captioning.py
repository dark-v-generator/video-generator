...
class Captioning(object) :
    ...
    def adjust_real_time_caption_text(self, text : str, is_recognized_result : bool) -> str :
        ...
        temp_caption_helper = caption_helper.CaptionHelper(self._config.language, self._config.max_line_length, self._config.lines, [])
    ...
    def caption_from_real_time_result(self, result : speechsdk.SpeechRecognitionResult, is_recognized_result : bool) -> Optional[str] :
        ...
        if self._previous_end_time is not None and self._previous_end_time > end_time :
            ...
            caption = caption_helper.Caption(self._config.language, self._srt_sequence_number, helper.add_time_and_timedelta(start_time, self._config.delay), helper.add_time_and_timedelta(end_time, self._config.delay), "")
            ...
            if self._previous_caption is not None :
                ...
                if self._previous_result_is_recognized :
                    ...
                    previous_end = helper.add_time_and_timedelta(self._previous_caption.end, self._config.get_remain_timedelta())
    ...
    def captions_from_offline_results(self) -> List[caption_helper.Caption] :
        captions = caption_helper.get_captions(self._config.language, self._config.max_line_length, self._config.lines, list(self._offline_results))
        # Save the last caption.
        last_caption = captions[-1]
        last_caption.end = helper.add_time_and_timedelta(last_caption.end, self._config.get_remain_timedelta())
        ...
        for (caption_1, caption_2) in pairwise(captions) :
            end = helper.add_time_and_timedelta(caption_1.end, self._config.get_remain_timedelta())
    ...
    def finish(self) -> None :
        if CaptioningMode.OFFLINE == self._config.captioning_mode :
            for caption in self.captions_from_offline_results() :
                helper.write_to_console_or_file(text=self.string_from_caption(caption), output_file=self._config.output_file, quiet=self._config.quiet)
            ...
            if self._previous_caption is not None :
                self._previous_caption.end = helper.add_time_and_timedelta(self._previous_caption.end, self._config.get_remain_timedelta())
                helper.write_to_console_or_file(text=self.string_from_caption(self._previous_caption), output_file=self._config.output_file, quiet=self._config.quiet)
    ...
    def initialize(self) :
        ...
        if not self._config.use_srt_format:
            helper.write_to_console_or_file(text="WEBVTT{}{}".format(linesep, linesep), output_file=self._config.output_file, quiet=self._config.quiet)
        return
    ...
    def audio_config_from_user_config(self) -> helper.Read_Only_Dict :
        if self._config.input_file is None :
            ...
            if self._config.format == 'wav':
                ...
                audio_stream_format = speechsdk.audio.AudioStreamFormat(compressed_stream_format=self._config.get_compressed_audio_format())
    ...
    def speech_config_from_user_config(self) -> speechsdk.SpeechConfig :
        speech_config = None
        speech_config = speechsdk.SpeechConfig(subscription=self._config.subscription_key, region=self._config.region)

        speech_config.set_profanity(self._config.get_profanity_option())

        if self._config.threshold is not None :
            speech_config.set_property(property_id=speechsdk.PropertyId.SpeechServiceResponse_StablePartialResultThreshold, value=str(self._config.threshold))
    ...
    def __get_speech_config(self):
        ...
        speech_config.set_profanity(self._config.get_profanity_option())
        if self._config.threshold is not None :
            speech_config.set_property(property_id=speechsdk.PropertyId.SpeechServiceResponse_StablePartialResultThreshold, value=self._config.threshold)

        speech_config.set_property(property_id=speechsdk.PropertyId.SpeechServiceResponse_PostProcessingOption, value="TrueText")
        speech_config.speech_recognition_language=self._config.language
        return speech_config
    
    def __get_speech_recognizer_data(self) -> helper.Read_Only_Dict :
    ...
    def recognize_continuous(self) :
        speech_recognizer_data = captioning.__get_speech_recognizer_data()
            ...
            if speechsdk.ResultReason.RecognizingSpeech == e.result.reason and len(e.result.text) > 0 :
                ...
                try :
                    ...
                    if caption is not None :
                        helper.write_to_console_or_file(text=caption, output_file=self._config.output_file, quiet=self._config.quiet)
                except Exception as ex :
                    print('Exception in recognizing_handler: {}'.format(ex))
            elif speechsdk.ResultReason.NoMatch == e.result.reason :
                helper.write_to_console(text="NOMATCH: Speech could not be recognized.{}".format(linesep), quiet=self._config.quiet)
        ...
        def canceled_handler(e : speechsdk.SpeechRecognitionCanceledEventArgs) :
            ...
            if speechsdk.CancellationReason.EndOfStream == e.cancellation_details.reason :
                helper.write_to_console(text="End of stream reached.{}".format(linesep), quiet=self._config.quiet)
                done = True
            elif speechsdk.CancellationReason.CancelledByUser == e.cancellation_details.reason :
                helper.write_to_console(text="User canceled request.{}".format(linesep), quiet=self._config.quiet)
                done = True
        ...
        def stopped_handler(e : speechsdk.SessionEventArgs) :
            nonlocal done
            helper.write_to_console(text="Session stopped.{}".format(linesep), quiet=self._config.quiet)
            done = True
